...
void Zadanie4();
void Zadanie5();
void bubbleSort(vector<Person> &Lista);
void Zadanie6();
int main()
{
	int numer_zadania;
...
void Zadanie4()
{
	...
	{
		file << I->firstName << " " << I->lastName << " " << I->age << " " << I->gender << endl;
	}
}
void Zadanie5()
{
	...
	vector<Person> Lista;
	double suma = 0;
	while (file >> Temp.firstName >> Temp.lastName >> Temp.age >> Temp.gender)
	{
	...
	cout << "Srednia wieku wynosi " << suma / Lista.size() << endl;
}
void bubbleSort(vector<Person> &Lista)
{
	for (int i = 0; i < Lista.size() - 1; i++)
	{
		for (int j = 0; j < Lista.size() - 1; j++)
			if (Lista[j].age > Lista[j + 1].age)
				swap(Lista[j].age, Lista[j + 1].age);
	}
}
void Zadanie6()
{
	//Zmodyfikowalem algorytm bubblesort tak aby dzialal prawidlowo dla Listy osob
	cout << "Zadanie 6" << endl;
	ifstream file;
	string name = "Zadanie4.txt";
	file.open(name.c_str());
	...
	cout << "Wczytuje liste z pliku" << endl;
	Person Temp;
	vector<Person> Lista;
	double suma = 0;
	while (file >> Temp.firstName >> Temp.lastName >> Temp.age >> Temp.gender)
	...
	cout << "Sortuje liste" << endl;
	bubbleSort(Lista);
	ofstream zfile;
	cout << "Zapis do pliku" << endl;
	string name2 = "Zadanie4 Po sortowaniu.txt";
	zfile.open(name2.c_str());
	if (!zfile.is_open())
	{
		cout << "Nie udalo sie otworzyc pliku o nazwie: " << name2 << endl;
		cout << "Byla to proba zapisu do pliku" << endl;
		system("pause");
		exit(EXIT_FAILURE);
	}
	for (vector<Person>::iterator I = Lista.begin(); I != Lista.end(); I++)
	{
		zfile << I->firstName << " " << I->lastName << " " << I->age << " " << I->gender << endl;
	}
}